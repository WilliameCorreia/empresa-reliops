# Template Postmortem

> Use este template como base para preencher `docs/postmortem-inc-001.md`.

## INC-[número]: [título do incidente]

- **Status:** Em análise | Concluído | Acompanhamento
- **Severidade:** SEV1 | SEV2 | SEV3
- **Período do incidente:** AAAA-MM-DD HH:MM até AAAA-MM-DD HH:MM
- **Autores:**
- **Times envolvidos:**

## Metadados de Governança
- Documento:
- Versão:
- Responsável (owner):
- Aprovador:
- Última atualização:
- Próxima revisão:
- Classificação da informação: Interna | Restrita | Confidencial

## Premissas, Lacunas e Riscos (preenchimento obrigatório)
- Premissas (o que está sendo assumido para elaborar o documento):
- Lacunas de informação (dados ausentes que impactam o detalhamento):
- Riscos identificados (inclua impacto e mitigação sugerida):


## 1. Resumo Executivo
- O que aconteceu:
- Impacto principal:
- Situação atual:

## 2. Impacto no Negócio e nos Usuários
- Serviços afetados:
- Janela de indisponibilidade/degradação:
- Impacto percebido pelo cliente:

## 3. Linha do Tempo do Incidente
| Horário | Evento | Evidência/Fonte | Responsável |
|---|---|---|---|
|   |   |   |   |

## 4. Detecção e Resposta Inicial
- Como o incidente foi detectado:
- Tempo até início da resposta:
- Medidas imediatas adotadas:

## 5. Causa Raiz e Fatores Contribuintes
- Causa raiz:
- Fatores técnicos contribuintes:
- Fatores processuais contribuintes:

## 6. Mitigação e Recuperação
- Ações que restauraram o serviço:
- Estratégia de rollback/contorno:
- Critério de estabilização:

## 7. Ações Corretivas e Preventivas
| Ação | Tipo (Corretiva/Preventiva) | Prioridade | Responsável | Prazo | Status |
|---|---|---|---|---|---|
|   |   |   |   |   |   |

## 8. Lições Aprendidas
- O que funcionou bem:
- O que não funcionou:
- Melhorias de processo recomendadas:

## 9. Acompanhamento
- Itens pendentes:
- Próxima revisão:
- Critério de encerramento do postmortem:


## Anexos e Referências
- Evidências (logs, métricas, gráficos):
- Canal/ticket do incidente:
- Runbooks, RFCs e ADRs relacionados:
- Links de PRs/issues das ações corretivas:

## Checklist de Qualidade (pré-entrega)
- [ ] Impacto e janela do incidente quantificados.
- [ ] Linha do tempo completa com evidências.
- [ ] Causa raiz e fatores contribuintes descritos.
- [ ] Ações corretivas/preventivas com dono e prazo.
- [ ] Premissas, lacunas, riscos e acompanhamento preenchidos.
